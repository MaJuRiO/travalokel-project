...
class TicketPDF(View):
    def get(self, request, pk):
        ...
        flight_detail = list(Flight.objects.select_related(#"flight_detail",
                                                            "flight_id","path_id").filter(flight_id=flight_id).values(
...
class FlightDetail(View):
    def get(self, request, id):
        flight = list(Flight.objects.select_related("flightclass","path").filter(flight_id=id).values('flight_id','airline','departure_time','arrival_time','path_id__departure','path_id__destination','flight_id__seat_class', 'flight_id__price'))
        flight_detail = list(Flight.objects.select_related("flight_id").filter(flight_id=id).values('flight_id','departure_date','gate_no'))
        data = dict()
